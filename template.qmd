...
```{r}
sfo_trees_df<-read_csv("data/sfo_trees/sfo_trees.csv") |> 
  janitor::clean_names()
```
...
```{r}
oak_pine_plot <- ggplot(oak_pine_df, aes(x=longitude, y=latitude, color=type))+
  theme_bw()+
  geom_point()
oak_pine_plot
```
...
sfo_trees_native<-sfo_trees_df |>
  separate(species, into=c('spp_sci','spp_common'),sep=' :: ') |> #separates species common and scientific names
...
# wrangle data so that you can input into a t.test  
...
```{r}
oak_pine_sf<-oak_pine_year_df |>   #sf = simple features object
  drop_na(longitude,latitude) |> 
  st_as_sf(coords=c("longitude","latitude"))

st_crs(oak_pine_sf)<-4326   #st_crs = set coord reference system
